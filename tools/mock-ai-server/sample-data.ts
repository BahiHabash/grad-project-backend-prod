...
const INJURY_RISKS = ['Low', 'Medium', 'High'] as const;
...
export function buildMockAnalysis(eventId: string, teamId: string) {
  return {
    event_id: eventId,
    team_id: teamId,
    ...
    match_context: {
      opponent_id: `team_${randInt(1, 999).toString().padStart(3, '0')}`,
      match_result: pick(MATCH_RESULTS),
      team_formation: pick(FORMATIONS),
    },

    players_analysis: PLAYERS.map((player) => ({
    ...
    trainingPlan: {
      teamDrills: TEAM_DRILLS.slice(0, randInt(2, 4)).map((drill) => ({
      ...
      individualDrills: INDIVIDUAL_DRILLS.slice(0, randInt(2, 4)).map((drill) => ({
